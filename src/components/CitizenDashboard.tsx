    ...
    const interval = setInterval(() => {
      setDrains((prevDrains) =>
        prevDrains.map((drain) => {
          let newWaterLevel =
            drain.waterLevel + Math.floor(Math.random() * 5) - 2;
        ...
        <div className="bg-red-500 text-white p-4 text-center shadow-lg animate-pulse">
          <div className="flex items-center justify-center gap-3">
            ...
            <p className="font-bold">
              FLOOD ALERT active in Surulere. Please be cautious.
            </p>
          </div>
        </div>
      )}
        ...
        {isAlertActive && (
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[1000]">
            <button
              ...
              className={`px-6 py-3 font-bold text-white rounded-full shadow-2xl transition-all duration-300 ${
                showSafeRoute ? "bg-blue-600" : "bg-green-600"
              }`}
            >
